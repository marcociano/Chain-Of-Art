...
contract PaintContract {
    ...
    function createPaint(
        ...
        paints[id] = Paint(id, title, img, artist, price, false);
        counterPaints++;
    }

    // Funzione per ottenere i dettagli di un quadro dato il suo ID
    function getPaintDetails(
        uint paintId
    )
        public
        view
    ...
    function paintExists(uint paintId) public view returns (bool) {
         return !paints[paintId].isSold;
    }

    // Funzione per acquistare un quadro
    function buyPaint(uint paintId) public payable {
        Paint storage paint = paints[paintId];
         require(paintExists(paintId), "Il quadro non e' disponibile");
        ...
        require(
            !purchasedStatus(msg.sender, paintId),
            "Il quadro non e' disponibile"
        );

        // Aggiorna lo stato del quadro come venduto
        paint.isSold = true;
    ...
    function getPurchasedPaints(
        address buyer
    ) public view returns (Paint[] memory) {
        uint[] memory purchasedIds = purchasedPaints[buyer];
